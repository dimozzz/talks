...
\begin{frame}{Strategy for the $\{0, 1\}$ basis ($\PCR[\field]$)}
    ...
    \begin{enumerate}
        ...
        \pause
        \only<-5>{\item Keep $\Fs \rest (x = 0)$ hard in terms of degree. \input{pics/cat.tex}}
        \only<6->{\item Try to avoid local contradictions in $\Fs \rest (x = 0)$. \input{pics/cat.tex}}
        \pause
        \pause    
...
\begin{frame}{Strategy for the $\{\pm 1\}$ basis ($\PCR[\field]$)}
    ...
    \begin{enumerate}
        ...
        \pause
        \item Try to avoid local contradictions in $\Split{x}(\Fs)$. \input{pics/cat.tex}
        \pause
...
\begin{frame}{Lazy computations \hspace{5pt} \input{pics/lazy.tex}}
    ...
    \pause
    \deftext{QR} of $\pi$: $(\lazy{p}_1^2, \dots, \lazy{p}_{\ell}^2)$. Squares are computed without
    cancellations.
...
\begin{frame}{QR and small degree}
