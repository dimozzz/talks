...
\begin{frame}{Circuits and protocols}
    ...
    \begin{theorem}[S 2016, unp.] 
        There is a {\color{blue}monotone} boolean circuit for function $f$ of size $S$ iff there is a boolean communication game of size
        $S$ for a relation $\Bit$ {\color{blue}$(\MBit)$} $(U = f^{-1}(1), V = f^{-1}(0))$.
    \end{theorem}
    ...
    \begin{columns}[t]
		\begin{column}{0.65\textwidth}
            ...
            \begin{enumerate}
                ...
                \item<6-> For an inner vertex $h$ with children $h', h''$: $f_h = f_{h'} \land f_{h''}$ or $f_h = f_{h'} \lor
                    f_{h''}$.
            \end{enumerate}
        \end{column}
...
\begin{frame}{Lower bounds}
    ...
    \begin{theorem}[Hrube{\v{s}}, Pudl{\'{a}}k 2016, unp.]
...
\begin{frame}{$\SAT_{G}$ function (Pitassi, G{\"{o}}{\"{o}}s, 2014)}
    ...
    \begin{itemize}
        ...
        \pause
        \item $\SAT_{G}: \{0, 1\}^N \to \{0, 1\}$, where $N \le |U| \cdot 2^d$ and $d$ is the maximum
